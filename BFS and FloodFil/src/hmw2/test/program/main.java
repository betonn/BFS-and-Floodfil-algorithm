            ...
            for (int a = 0; a < width; a++)
                if (Maze[i][a] == 0)
                    if (i == 0 || a == 0 || i == (height - 1)
                            || a == (width - 1)) {
                        hmw2.program.FloodFill ff = new hmw2.program.FloodFill();
...
//--------------------------------------------------------
        ...
        for (int i = 0; i < height; i++)
            for (int j = 0; j < width; j++)
                if (Maze[i][j] == 0) {
                    cycles++;
                    hmw2.program.FloodFill cycle = new FloodFill();
                    cycle.pathLength = 0;
    ...
    public static void main(String[] args) {
        ...
        String source ="C:\\Users\\m_722\\IdeaProjects\\graphs\\src\\hmw2\\test\\program\\input.txt";
        main m = new main(); // main object
        m.reader(source); // reads file
        int Maze[][] = new int[height * 2][width * 2]; // creates 2d array
        m.CreateMaze(Maze); // creates 2d array maze
        m.boundry(Maze); // handle boundries
        m.result(Maze); // returns result.
